...
describe('ActivateScene Command', () => {
  ...
  describe('convertParamsToValue', () => {
    ...
    test('convertParamsToValue inverted', () => {
      expect(
        new Command({ deactivate: true }, { id: 'Item', customData: { inverted: true } }).convertParamsToValue()
      ).toBe('ON');
      expect(
        new Command({ deactivate: false }, { id: 'Item', customData: { inverted: true } }).convertParamsToValue()
      ).toBe('OFF');
    });
  });
});
